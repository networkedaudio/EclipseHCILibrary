...
/// <summary>
/// Represents a single remote key action status entry.
/// </summary>
public class RemoteKeyActionStatusEntry
{
    /// <summary>
...
public class ReplyRemoteKeyActionStatus
{
    ...
    /// <summary>
    /// Endpoint type associated with the port.
    /// Only set to a non-zero value when the port is a role.
    /// Indicates which endpoint the role is currently logged into.
    /// </summary>
    public ushort EndpointType { get; set; }

    /// <summary>
    /// List of remote key action status entries.
    /// </summary>
    public List<RemoteKeyActionStatusEntry> Actions { get; set; } = new();

    /// <summary>
    /// Parses the payload of a Reply Remote Key Action Status message.
    /// </summary>
    /// <param name="payload">The message payload (after protocol schema byte).</param>
    /// <returns>The parsed ReplyRemoteKeyActionStatus, or null if parsing fails.</returns>
    public static ReplyRemoteKeyActionStatus? Parse(byte[] payload)
    {
        // Minimum payload: Type(1) + Count(2) + PortNumber(2) + EndpointType(2) = 7 bytes
        if (payload == null || payload.Length < 7)
        {
            return null;
        }

        var result = new ReplyRemoteKeyActionStatus();
        int offset = 0;
        ...
        // Endpoint Type: 2 bytes (big-endian)
        ...
        for (int i = 0; i < result.Count; i++)
        {
            ...
            var entry = new RemoteKeyActionStatusEntry();
