...
public class HCIService : IAsyncDisposable
{
    ...
    /// <summary>
    /// Sends a request via the underlying HCI connection's request queue.
    /// Does nothing if not connected.
    /// </summary>
    /// <param name="request">The request to send.</param>
    public void SendRequest(HCIRequest request)
    {
        _connection?.RequestQueue?.Enqueue(request);
    }

    /// <summary>
    /// Requests crosspoint status for the specified ports.
    /// </summary>
    /// <param name="ports">The port numbers to monitor (0-1023 each).</param>
    public void RequestCrosspointStatus(IEnumerable<ushort> ports)
    {
        var request = new RequestCrosspointStatusRequest();
        request.AddPorts(ports);
        SendRequest(request);
    }

    /// <summary>
    /// Requests conference (partyline) status for the specified conference number.
    /// </summary>
    /// <param name="conferenceNumber">The conference number to query.</param>
    public void RequestConferenceStatus(ushort conferenceNumber)
    {
        SendRequest(new RequestConferenceStatusRequest(conferenceNumber));
    }

    /// <summary>
    /// Requests card info and health status for all slots (slot 0 = all cards).
    /// </summary>
    public void RequestCardInfo()
    {
        System.Diagnostics.Debug.WriteLine("[HCIService.RequestCardInfo] Sending RequestCardInfoRequest for all slots");
        SendRequest(new RequestCardInfoRequest());
    }
    ...
    /// reply-generation loop for an "All" request (entity type EN_NULL == 0). A single-type
    /// request (e.g. Conferences == 2) leaves currentEntity != EN_NULL, so the firmware's
    /// do/while loop never exits and the matrix hangs (watchdog reset).
    ///
    /// Always request ALL entity types and filter the reply client-side.
    /// See <see cref="ReplyEntityInfo.Conferences"/>, <see cref="ReplyEntityInfo.Groups"/>
    /// and <see cref="ReplyEntityInfo.Ifbs"/>.
    /// </summary>
    public void RequestAllEntities()
    {
        System.Diagnostics.Debug.WriteLine("[HCIService.RequestAllEntities] Sending RequestEntityInfoRequest for ALL entities (firmware-safe)");
        SendRequest(RequestEntityInfoRequest.CreateForAll());
    }

    /// <summary>
    /// Requests conference entity labels from the matrix.
    /// Retained for backwards compatibility; delegates to <see cref="RequestAllEntities"/>.
    /// </summary>
    public void RequestConferenceEntities() => RequestAllEntities();
